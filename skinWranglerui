...
class skinWrangler(QtWidgets.QDialog):
    ...
    def findRelatedSkinCluster(self, skinObject):
        ...
        for c in clusters:
            ...
            if geom is None:  # can find deformers not skin clusters. 
                continue
            for g in geom:
    ...
    def getSelected(self):
        ...
        if msh:
            ...
            if not skin:
                # cmds.warning('Cannot find a skinCluster related to [' + msh[0] + ']')
                return False
    ...
    def setAverageWeightFn(self):
        try:
            ...
            if not self.ui.avgOptionCHK.isChecked():
                ...
                self.clampInfluences(self.currentMesh, self.ui.clampInfSPIN.value(), force=1)
                cmds.select(sel)
        except Exception as e:
    ...
    def jointOnBboxCenterFn(self):
        if self.ui.jointOnBboxCenterBTN.isChecked() == True:
    ...
    def refreshUI(self):
        ...
        skin = None
        if self.currentMesh: self.ui.mshLBL.setText(self.currentMesh)
        if self.currentSkin:
        ...
        if skin:
            ...
            if m == 0: self.ui.skinAlgoLBL.setText('Linear')
            if m == 1: self.ui.skinAlgoLBL.setText('DualQuat')
            if m == 2: self.ui.skinAlgoLBL.setText('Blended')
            ...
            for inf in wDict.keys():
                if filter in inf.lower() or filter == '':
                    wid = QtWidgets.QTreeWidgetItem()
                    infName = inf
                    if self.ui.nameSpaceCHK.isChecked(): infName = inf.split(':')[-1]
                    ...
                    self.ui.jointLST.addTopLevelItem(wid)
            if self.ui.listAllCHK.isChecked() == True:
                for inf in cmds.skinCluster(self.currentSkin, q=1, inf=1):
                    if inf not in wDict.keys():
                        if filter in inf.lower() or filter == '':
                            wid = QtWidgets.QTreeWidgetItem()
                            wid.setIcon(0, self.iconLib['joint'])
                            if self.ui.nameSpaceCHK.isChecked(): inf = inf.split(':')[-1]
                            wid.setText(0, inf)
                            self.ui.jointLST.addTopLevelItem(wid)
